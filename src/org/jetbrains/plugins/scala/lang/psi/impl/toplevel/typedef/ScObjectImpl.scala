...
class ScObjectImpl protected (stub: StubElement[ScTemplateDefinition], nodeType: IElementType, node: ASTNode)
  ...
  def fakeCompanionClass: Option[PsiClass] = {
    val clazz = companionModule(checkSynthetic = false) match {
  ...
  def fakeCompanionClassOrCompanionClass: PsiClass =
    fakeCompanionClass.orElse(companionModule(checkSynthetic = false)).get
  ...
  private def companionModule(checkSynthetic: Boolean = true): Option[ScTypeDefinition] =
    if (checkSynthetic && isSyntheticObject) ScalaPsiUtil.getBaseCompanionModule(this) else None
}
